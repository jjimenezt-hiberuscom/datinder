      ...
      {areaCompats.length > 0 && (
        <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {areaCompats.map(c => (
              <div key={`${c.areaA}-${c.areaB}`} className="bg-white/5 rounded-xl p-3 text-center">
                <p className="text-white/60 text-xs mb-1 leading-tight">{c.label}</p>
                <p className={`font-black text-2xl tabular-nums ${c.afinidadMedia >= 70 ? 'text-datinder-yellow' : c.afinidadMedia >= 50 ? 'text-white' : 'text-white/50'}`}>
                  {Math.round(c.afinidadMedia)}%
                </p>
                <p className="text-white/30 text-xs mt-1">{c.numParejas} {c.numParejas === 1 ? 'pareja' : 'parejas'}</p>
              </div>
            ))}
          </div>
        </div>
